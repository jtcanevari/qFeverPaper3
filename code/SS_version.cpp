...
/// --------------------------------------------------------------------------
/// define forcing function Qc(t)onceivings

double tc = 365 + tp - 150; //day conception
...
/// --------------------------------------------------------------------------
/// define forcing function Q4(t) for herd A movement to IP2
double Tl = 4*7;
...
// [[Rcpp::export]]
Rcpp::List Gillespie(double t_start, double t_end, std::vector<double> n_init, std::vector<double> par)
{
  double t=t_start, phi=par[0], mu=par[1], fI=par[2], gamma=par[3], alpha=par[7], beta=par[8], iota=par[9], p=par[10], epsilon_p=par[11], epsilon_f=par[12], muE=par[13], u=0.0; 
  
  double rates[32], E=n_init[16];
  
  int SNP=n_init[0], SP=n_init[1], INP1=n_init[2], INP2=n_init[3], INP3=n_init[4], INP4=n_init[5], IP=n_init[6], IP2=n_init[7], JNP=n_init[8], JP=n_init[9], RNP=n_init[10], RP=n_init[11], Y=n_init[12], A=n_init[13], K=n_init[14], KI=n_init[15], i=0;
...
NumericVector sheds = NumericVector::create(1.0, 0.0001);
NumericVector probs = NumericVector::create(0.2, 0.8);
  
//  IntegerVector SNP_vec, SP_vec, INP_vec, IP_vec, RNP_vec, RP_vec, N_vec;
  std::vector<int> SNP_vec, SP_vec, INP1_vec, INP2_vec, INP3_vec, INP4_vec, IP_vec, IP2_vec, JNP_vec, JP_vec, RNP_vec, RP_vec, Y_vec, A_vec, K_vec, KI_vec, N_vec;
  ...
  //while(t<t_end && nB < n_MAX[0] && nL < n_MAX[1] && nS < n_MAX[2]){
  ...
  while(t<t_end){
    ...
    if((phi*(SNP+SP)-SP) > 0) {
      rates[1] = (phi*(SNP+SP)-SP)*Qc(t) + rates[0];
      } else {
      rates[1] = exp(-10) + rates[0];
      };
    
    if((phi*(INP1+IP/4)-IP/4) > 0) {
      rates[2] = (phi*(INP1+IP/4)-IP/4)*Qc(t) + rates[1];
      } else {
      rates[2] = exp(-10) + rates[1];
      };

    if((phi*(INP2+IP/4)-IP/4) > 0) {
      rates[3] = (phi*(INP2+IP/4)-IP/4)*Qc(t) + rates[2];
      } else {
      rates[3] = exp(-10) + rates[2];
      };
    
    if((phi*(INP3+IP/4)-IP/4) > 0) {
      rates[4] = (phi*(INP3+IP/4)-IP/4)*Qc(t) + rates[3];
      } else {
      rates[4] = exp(-10) + rates[3];
      };

    if((phi*(INP4+IP/4)-IP/4) > 0) {
      rates[5] = (phi*(INP4+IP/4)-IP/4)*Qc(t) + rates[4];
      } else {
      rates[5] = exp(-10) + rates[4];
      };

    if((phi*(RNP+RP)-RP) > 0) {
      rates[6] = (phi*(RNP+RP)-RP)*Qc(t) + rates[5];
      } else {
      rates[6] = exp(-10) + rates[5];
      };

    if((phi*(JNP+JP)-JP) > 0) {
      rates[7] = (phi*(JNP+JP)-JP)*Qc(t) + rates[6];
      } else {
      rates[7] = exp(-10) + rates[6];
      };
    
    rates[8] = SP*Qp(t) + rates[7];
        
    rates[9] = (1-alpha)*IP*Qp(t) + rates[8];
    
    rates[10] = alpha*IP*Qp(t) + rates[9];
        
    rates[11] = (1-alpha)*IP2*Qp(t) + rates[10];
        
    rates[12] = alpha*IP2*Qp(t) + rates[11];
    
    rates[13] = JP*Qp(t) + rates[12];
    
    rates[14] = RP*Qp(t) + rates[13];
    
    rates[15] = mu*SNP + rates[14];
    
    rates[16] = mu*INP1 + rates[15];
    
    rates[17] = mu*INP2 + rates[16];
        
    rates[18] = mu*INP3 + rates[17];
    
    rates[19] = mu*INP4 + rates[18];
    
    rates[20] = mu*JNP + rates[19];
     
    rates[21] = mu*RNP + rates[20];
    
    rates[22] = (beta*SNP*E)/N + rates[21];
    
    rates[23] = Q4(t)*beta*SP*E/N + rates[22];
    
    rates[24] = beta*(1-Q4(t))*SP*E/N + rates[23];
    
    rates[25] = gamma*INP1 + rates[24];
    
    rates[26] = gamma*INP2 + rates[25];
    
    rates[27] = gamma*INP3 + rates[26];
    
    rates[28] = (1-p)*gamma*INP4 + rates[27];
    
    rates[29] =  p*gamma*INP4 + rates[28];
    
    rates[30] = alpha*fI*IP*Qa(t) + rates[29];
    
    rates[31] =  (1-alpha)*fI*IP*Qa(t)+ rates[30];
    
//    rates[32] = fJ*JP*Qa(t) + rates[31];
    
    // 2. Draw the time to next event
    
    double tstep = R::rexp(1/rates[31]); //should add small number to prevent ceros in rate[32]?
    
    if(tstep < 14){
    
      // 3. Draw the next event
    
      u = R::runif(0,1)*rates[31];
      ...
      while(rates[i]<u) i++;
        ...
        //case 9: IP--; RNP++; K++; KI++; E += RcppArmadillo::sample(sheds, 1, TRUE,probs); break;
